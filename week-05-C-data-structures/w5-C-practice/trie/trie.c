...
{
    bool found = false;
    // sets a node pointer, cursor to the root of the trie.
    node *cursor = root;
    // Iterate through every letter in the argument word and as it does, it determines the array index that corresponds to that letter.
    for (int i = 0, length = strlen(word); i < length; i++)
    {
        // the lookup should be case-insensitive. For instance, A and a should correspond to 0, B and b corresponds to 1, etc. So...
        int index = tolower(word[i]) - 'a';
        // use the index for a letter to check if children[index] is a NULL pointer, meaning the word does not exist in the trie.
        if (cursor->children[index] != NULL)
        {
            // If children[index] is in fact a node, resets cursor to this node and checks for the next letter in its children nodes.
            cursor = cursor->children[index];
            found = true;
        }
        else
